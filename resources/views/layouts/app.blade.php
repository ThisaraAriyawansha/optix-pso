...
<head>
    ...
    <title>@yield('title', 'Dashboard') — OptiX POS</title>
...
<body class="bg-[#F5F7FA] font-body" x-data="{ sidebarOpen: true, mobileMenuOpen: false }">
...
<header class="fixed top-0 left-0 right-0 h-14 bg-white border-b border-[#E2E8F0] z-30 flex items-center px-4 gap-4">
    ...
    <div class="flex items-center gap-3">
        ...
        <a href="{{ route('dashboard') }}" class="flex items-center gap-1 font-body font-semibold text-xl text-[#1A202C]">
    ...
    {{-- Search --}}
    <button class="hidden md:flex items-center gap-2 px-3 py-1.5 text-sm text-[#64748B] bg-[#F5F7FA] border border-[#E2E8F0] rounded-md hover:border-[#004080] transition-colors">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
        </svg>
        <span>Search...</span>
        <kbd class="text-xs bg-white border border-[#E2E8F0] rounded px-1">⌘K</kbd>
    </button>

    {{-- Notifications --}}
    <button class="relative p-2 text-[#64748B] hover:text-[#004080] hover:bg-[#F5F7FA] rounded-md transition-colors">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-data="{ open: false }" class="relative">
        ...
        <div x-show="open" @click.away="open = false" x-transition
            ...
            <a href="#" class="flex items-center gap-2 px-4 py-2 text-sm text-[#64748B] hover:bg-[#F5F7FA] hover:text-[#1A202C]">
...
{{-- MAIN CONTENT --}}
<main :class="sidebarOpen ? 'ml-56' : 'ml-16'" class="mt-14 mb-10 transition-all duration-200 min-h-[calc(100vh-6rem)]">
    {{-- Flash Messages --}}
    @if(session('success'))
        <div class="mx-6 mt-4">
...
<footer class="fixed bottom-0 left-0 right-0 h-10 bg-[#004080] text-white flex items-center px-4 gap-3 z-30 text-xs">
    ...
    <div class="flex-1 text-center text-white/70" id="today-stats">
